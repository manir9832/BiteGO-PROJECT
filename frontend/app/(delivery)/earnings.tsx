...
export default function DeliveryEarnings() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.surface, paddingTop: insets.top + S.md }}>
      <Txt weight="semibold" size={T["2xl"]} style={{ paddingHorizontal: S.lg }}>Earnings</Txt>
      <View style={styles.grid}>
        {/* ১. বকেয়া বা পে-আউট পাওয়ার মতো ব্যালেন্স (অ্যাডমিন পে করলে এটি 0 হয়ে যাবে) */}
        
        <Card style={styles.stat}>
          <Txt weight="semibold" size={T.xl} color={C.primary}>{money(data?.pending_payout ?? 0)}</Txt>
          <Txt size={T.sm} color={C.muted}>Pending Payout</Txt>
        </Card>
        <Card style={styles.stat}>
          <Txt weight="semibold" size={T.xl} color={C.success}>{money(data?.today_earnings)}</Txt>
          <Txt size={T.sm} color={C.muted}>Today's Earnings</Txt>
        </Card>

        <Card style={styles.stat}>
          <Txt weight="semibold" size={T.xl}>{data?.today_deliveries ?? 0}</Txt>
          <Txt size={T.sm} color={C.muted}>Today's Deliveries</Txt>
        </Card>

        <Card style={styles.stat}>
          <Txt weight="semibold" size={T.xl}>{money(data?.total_earnings)}</Txt>
          <Txt size={T.sm} color={C.muted}>All-Time Earnings</Txt>
        </Card>
      </View>
