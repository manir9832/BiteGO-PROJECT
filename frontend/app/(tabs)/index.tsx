...
//         api.get("/categories"),
//         api.get("/restaurants", { lat, lng, category: category || undefined }),
...
//             <Txt weight="semibold" numberOfLines={1}>{areaName || loc?.address || "Select Location"}</Txt>
//           </View>
    ...
    <View style={[styles.headerWrap, { paddingTop: insets.top + S.sm }]}>
      ...
      <View style={styles.headerRow}>
        ...
        >
          ...
          <View style={{ maxWidth: 220 }}>
            <Txt size={T.sm} color={C.muted}>Deliver to</Txt>
            {/* 👇 এখানে লাইভ লোকেশন অ্যাড্রেসকে আগে প্রাধান্য দেওয়া হয়েছে যাতে ফিক্সড বা ভুল নাম না দেখায় */}
            <Txt weight="semibold" numberOfLines={1}>{loc?.address || areaName || "Select Location"}</Txt>
          </View>
          <Ionicons name="chevron-down" size={16} color={C.onSurface} />
        </Pressable>
        <Pressable style={styles.bell} onPress={() => router.push("/notifications")} testID="home-notifications">
