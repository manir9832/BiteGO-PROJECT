...
export default function OrderDetail() {
  ...
  const curIdx = TRACK_STEPS.includes(o.status)
    ? TRACK_STEPS.indexOf(o.status)
    : 0;

  const timelineMap: Record<string, string> = {};

  if (Array.isArray(o.timeline)) {
    o.timeline.forEach((t: any) => {
      if (t && t.status && !timelineMap[t.status]) {
        timelineMap[t.status] = t.at;
      }
    });
  }

  const cancel = async () => {
    // Customer can cancel only while PLACED or ACCEPTED.
    // Backend also enforces this rule.
    if (!["PLACED", "ACCEPTED"].includes(o.status)) {
      toast.show(
        "Delivery partner has accepted this order. You can no longer cancel it.",
        "error"
      );
      return;
    }
  ...
  const submitReview = async () => {
    ...
    try {
      await api.post(`/orders/${id}/review`, {
        restaurant_rating: stars,
        delivery_rating: stars,
        comment,
      });
  ...
  // Safe polyline decoding to prevent crashes
  // if route_polyline is missing/invalid
  let routeCoords: {
    latitude: number;
    longitude: number;
  }[] = [];
  ...
  try {
    if (o.route_polyline) {
      routeCoords = decodePolyline(o.route_polyline);
    }
  } catch (err) {
    ...
    <View style={styles.root}>
      ...
      >
        <View style={styles.statusHead}>
          <Txt weight="semibold" size={T["2xl"]}>
              ...
              <AppMap
                ...
                markers={[
                  ...
                  ...(o.partner_location
                    ? [
                        {
                          lat: o.partner_location.lat,
                          lng: o.partner_location.lng,
                          title:
        ...
        {["PLACED", "ACCEPTED"].includes(o.status) ? (
          ...
          <View style={{ marginTop: S.md }}>
            <Button
              ...
              disabled={true}
              style={{ opacity: 0.45 }}
              testID="cancel-order-button-disabled"
            />
            ...
            >
              Delivery partner has accepted this
              order. You can no longer cancel it.
            </Txt>
          </View>
        ) : null}
        ...
        <View style={styles.invoice}>
          ...
          <InvRow
            label="Item Total"
            value={money(o.food_subtotal || 0)}
          />

          <InvRow
            label="Platform Charge"
            value={money(o.platform_charge || 0)}
          />

          <InvRow
            label="Delivery Charge"
            value={money(
              o.customer_delivery_charge || 0
            )}
          />
