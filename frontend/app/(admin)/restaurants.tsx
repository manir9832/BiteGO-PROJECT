...
export default function AdminRestaurants() {
  ...
  const openRestaurantControl = (
    restaurant: any
  ) => {
    router.push({
      pathname: "/restaurant-control",
      params: {
        id: restaurant.id,
    ...
    >
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          subtitle="Restaurant registrations will appear here."
        />
      ) : (
        <FlatList
          data={rows}
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View
                style={styles.card}
                testID={`admin-restaurant-${item.id}`}
              >
                <ImageUpload
                  variant="banner"
                  value={
