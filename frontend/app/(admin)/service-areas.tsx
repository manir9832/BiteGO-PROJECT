...
export default function AdminServiceAreas() {
  ...
  const load = useCallback(async () => {
    try { 
      const r = await api.get("/admin/service-areas"); 
      setRows(r.areas || []); 
    } catch (e: any) {
      toast.show(e?.message || "Failed to load service areas", "error");
    } finally { 
      setLoading(false); 
    }
  }, [toast]);
  ...
  const openEdit = (a: any) => { 
    ...
    setForm({ 
      name: a.name || "", 
      lat: String(a.lat ?? "22.5726"), 
      lng: String(a.lng ?? "88.3639"), 
      radius_km: String(a.radius_km ?? "10"), 
      priority_radius_km: String(a.priority_radius_km ?? "5"), 
  ...
  const save = async () => {
    ...
    const body = { 
      name: form.name.trim(), 
      lat: parseFloat(form.lat) || 22.5726, 
      lng: parseFloat(form.lng) || 88.3639, 
      radius_km: parseFloat(form.radius_km) || 10, 
      priority_radius_km: parseFloat(form.priority_radius_km) || 5, 
      active: form.active 
    };
    try {
      if (editId) await api.put(`/admin/service-areas/${editId}`, body);
      else await api.post("/admin/service-areas", body);
      toast.show("Service area saved", "success"); 
      setModal(false); 
      load();
    } catch (e: any) { 
  ...
  const toggleActive = async (a: any) => {
    try { 
      await api.put(`/admin/service-areas/${a.id}`, { 
        name: a.name, 
        lat: a.lat, 
        lng: a.lng, 
        radius_km: a.radius_km, 
        priority_radius_km: a.priority_radius_km, 
        active: !a.active 
      }); 
      load(); 
    } catch (e: any) { 
  ...
  const remove = async (id: string) => {
    try { 
      await api.del(`/admin/service-areas/${id}`); 
      toast.show("Area deactivated (history preserved)", "success"); 
      load(); 
    } catch (e: any) { 
      toast.show(e.message, "error"); 
    }
      ...
      {loading ? <Loading /> : rows.length === 0 ? (
        ...
        <ScrollView contentContainerStyle={{ padding: S.lg, paddingBottom: S["3xl"], gap: S.md }}>
          <View style={styles.overviewMap} testID="areas-overview-map">
            <AppMap
              style={{ height: 300 }}
              markers={rows.filter((a) => a.active && a.lat && a.lng).map((a) => ({ lat: a.lat, lng: a.lng, title: a.name, color: C.brandPrimary }))}
              circles={rows.filter((a) => a.active && a.lat && a.lng).map((a) => ({ lat: a.lat, lng: a.lng, radiusKm: a.radius_km, color: C.brandPrimary }))}
            />
          </View>
          {rows.map((a) => (
            <Card key={a.id} style={styles.card} testID={`area-${a.id}`}>
              ...
              <AppMap
                ...
                onPress={(coords) => {
                  if (coords && coords.lat && coords.lng) {
                    setForm((f: any) => ({ ...f, lat: coords.lat.toFixed(6), lng: coords.lng.toFixed(6) }));
                  }
                }}
                markers={form.lat && form.lng ? [{ lat: parseFloat(form.lat), lng: parseFloat(form.lng), title: form.name || "Center", color: C.brandPrimary }] : []}
                circles={form.lat && form.lng && form.radius_km ? [{ lat: parseFloat(form.lat), lng: parseFloat(form.lng), radiusKm: parseFloat(form.radius_km) || 0, color: C.brandPrimary }] : []}
