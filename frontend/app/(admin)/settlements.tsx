...
export default function AdminSettlements() {
  ...
  const load = useCallback(async () => {
    try { 
      setLoading(true);
      // নতুন ডিটেইলড সেটেলমেন্ট এন্ডপয়েন্ট কল করা হলো
      ...
      {restaurants.length === 0 ? (
        ...
        restaurants.map((r: any) => (
          <Card key={r.id} style={styles.card} testID={`settle-rest-${r.id}`}>
            ...
            <Row l="Today's Earnings" v={money(r.daily_earnings)} tint={C.success} />
            <Row l="Weekly Earnings" v={money(r.weekly_earnings)} tint={C.info} />
            <View style={styles.divider} />
            <Row l="Remaining Due" v={money(r.remaining)} tint={C.warning} bold />
      ...
      {partners.length === 0 ? (
        ...
        partners.map((p: any) => (
          <Card key={p.id} style={styles.card} testID={`settle-partner-${p.id}`}>
            ...
            <Row l="Today's Earnings" v={money(p.daily_earnings)} tint={C.success} />
            <Row l="Weekly Earnings" v={money(p.weekly_earnings)} tint={C.info} />
            <View style={styles.divider} />
            <Row l="Remaining Due" v={money(p.remaining)} tint={C.warning} bold />
