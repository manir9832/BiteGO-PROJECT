...
export function FoodRow({ f, qty, onInc, onDec }:
  ...
  // ডিসকাউন্ট চেক করার লজিক
  const hasDiscount = f.discount_price != null && f.discount_price < f.price;
  const effectivePrice = hasDiscount ? f.discount_price : (f.price || 0);
  ...
  return (
    <View style={styles.food} testID={`food-row-${f.id}`}>
      <View style={{ flex: 1, paddingRight: S.md }}>
        ...
        {/* ডিসকাউন্ট থাকলে কাটা দাগ সহ আসল দাম এবং নতুন দাম শো করবে */}
        {hasDiscount ? (
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginVertical: 2 }}>
            <Txt weight="medium" color={C.onSurface}>{money(effectivePrice)}</Txt>
            <Txt size={T.sm} color={C.muted} style={{ textDecorationLine: "line-through" }}>
              {money(f.price)}
            </Txt>
          </View>
        ) : (
          <Txt weight="medium" style={{ marginVertical: 2 }}>{money(f.price)}</Txt>
        )}
