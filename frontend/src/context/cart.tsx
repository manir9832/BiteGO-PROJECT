...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const applyAdd = (state: CartState, food: any, restaurant: { id: string; name: string }): CartState => {
    ...
    // ডিসকাউন্ট চেক করে সঠিক দাম নির্ধারণের লজিক
    const hasDiscount = food.discount_price != null && food.discount_price < food.price;
    const effectivePrice = hasDiscount ? food.discount_price : (food.price || 0);

    if (idx >= 0) lines[idx] = { ...lines[idx], quantity: lines[idx].quantity + 1 };
    else lines.push({ food_id: food.id, name: food.name, price: effectivePrice, quantity: 1, image: food.image });
  ...
  const setQty = (food: any, restaurant: { id: string; name: string }, qty: number) => {
    ...
    setCart((s) => {
      ...
      if (idx >= 0) lines[idx] = { ...lines[idx], quantity: qty };
      else lines.push({ food_id: food.id, name: food.name, price: effectivePrice, quantity: qty, image: food.image });
