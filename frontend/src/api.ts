...
// BiteGo API client — Base64 upload to bypass all Android FormData and Expo issues
import { Platform } from "react-native";
import { storage } from "@/src/utils/storage";
...
// Bulletproof upload using fetch blob / base64 without any FormData or FileSystem module
export async function uploadImage(
  imageSource: any, 
  name = "image.jpg", 
  type = "image/jpeg",
  ...
  } else if (imageSource && typeof imageSource.uri === "string") {
  ...
  // Fetch image as blob directly and send via standard fetch POST
