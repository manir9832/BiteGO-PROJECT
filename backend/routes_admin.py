...
async def force_reset_admin():
    hashed = pwd_context.hash("2026@Admin") # অথবা সরাসরি os.getenv("FIRST_ADMIN_PASSWORD") দিতে পারেন
    await db.users.update_one(
        {"email": "admin@bitego.com"},
        {"$set": {"email": "admin@bitego.com", "password_hash": hashed, "role": "admin", "name": "Super Admin"}},
        upsert=True
    )
    return {"success": True, "message": "Admin reset successfully!"}
...
class SettingsUpdate(BaseModel):
    ...
    priority_radius_km: Optional[float] = None
    ordering_enabled: Optional[bool] = None
    opening_time: Optional[str] = None      # নতুন যোগ করা হলো: ওপেনিং টাইম
    closing_time: Optional[str] = None      # নতুন যোগ করা হলো: ক্লোজিং টাইম
    helpline: Optional[str] = None
...
async def admin_orders(
    ...
    # ১. স্ট্যাটাস ফিল্টার (যদি 'All' বা খালি না থাকে)
    if status and status.lower() != "all":
        query["status"] = status.upper()

    # ২. সার্চ কুয়েরি হ্যান্ডেলিং (অর্ডার আইডি বা অন্য কিছু দিয়ে সার্চ করলে)
    if q:
        clean_q = q.strip().replace("#", "") # হ্যাশ সাইন রিমুভ করা হলো
        # এখানে অবজেক্ট আইডি অথবা স্ট্রিং আইডি এবং কাস্টমার নাম দুটোই চেক করা হবে
        search_conditions = [
            {"customer_name": {"$regex": clean_q, "$options": "i"}},
            {"customer_phone": {"$regex": clean_q, "$options": "i"}}
        ]
        # যদি কুয়েরিটি সঠিক ফরম্যাটের হয় তবে _id দিয়েও খোঁজার চেষ্টা করবে
        try:
            search_conditions.append({"_id": oid(clean_q)})
        except:
            pass
            
        query["$or"] = search_conditions

    # পেজিনেশন ও ডাটা ফেচিং (প্রয়োজনে লিমিট বাড়িয়ে বা পেজিনেশন হ্যান্ডেল করা)
