...
function ensureGoogle(): Promise<void> {
  if (!ready) {
    setOptions({
      key: process.env.EXPO_PUBLIC_GOOGLE_MAPS_WEB_KEY as string,
      v: "weekly",
    });
...
export default function AppMap({
  ...
  // Keep Google Maps runtime object untyped here because
  // the Google Maps namespace is loaded dynamically in the browser.
  ...
  function draw() {
    ...
    if (
      fitToMarkers &&
      (markers.length > 1 || circles.length > 0)
    ) {
