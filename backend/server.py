                ...
                result = await db.orders.update_one(
                    ...
                    },
                    {
                        "$set": {
                            ...
                            "cancellation_reason": "Restaurant did not accept in time",
...
async def create_order(body: CreateOrder, user=Depends(require_roles("customer"))):
    ...
    # রেস্টুরেন্ট খোলার স্ট্যাটাস চেক
    if not restaurant.get("is_open", True):
        raise HTTPException(409, "Restaurant is currently closed")

    # নির্দিষ্ট টাইম-বেসড অপারেটিং আওয়ার্স চেক (যদি ডেটাবেজে opening_time ও closing_time থাকে)
    current_time_str = now().strftime("%H:%M")
    open_time = restaurant.get("opening_time")
    close_time = restaurant.get("closing_time")
    if open_time and close_time:
        if not (open_time <= current_time_str <= close_time):
            raise HTTPException(409, f"Restaurant is closed now. Operating hours: {open_time} to {close_time}")
...
@app.get("/")
def read_root():
    return {"message": "Backend is running successfully!", "docs": "/docs"}
...
#         if not config.RENFLAIR_API_KEY:
#             await db.otp_challenges.update_one(
#                 {"_id": result.inserted_id}, {"$set": {"consumed": True}}
#             )
#             raise HTTPException(500, "Renflair API key is not configured")
...
#             if response.is_error:
#                 logger.error(
#                     "Renflair SMS failed: status=%s body=%s",
...
#                     {"$set": {"consumed": True}}
#                 )
#                 raise HTTPException(502, "Unable to send OTP SMS. Please try again.")

#             logger.info(
#                 "OTP SMS request sent successfully to phone ending %s",
#                 phone[-4:]
#             )
...
#             raise
#         except Exception as e:
#             logger.error("Renflair SMS error: %s", e)
#             await db.otp_challenges.update_one(
#                 {"_id": result.inserted_id},
#                 {"$set": {"consumed": True}}
#             )
