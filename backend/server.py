...
#     """Cancel stale restaurant orders after the configured timeout and notify customer once."""

#     while True:
#         try:
#             cutoff = now() - timedelta(
#                 minutes=RESTAURANT_ACCEPT_TIMEOUT_MIN
#             )
...
#                     "timeout_notification_sent": {"$ne": True},
#                 }
#             )
...
#                             "cancellation_reason": (
#                                 "Restaurant did not accept in time"
#                             ),
...
#                 # Only the worker that successfully cancelled the order
#                 # is allowed to send the customer notification.
...
#             "expires_at": now()
#             + timedelta(seconds=config.OTP_TTL_SEC),
#             "resend_at": now()
#             + timedelta(seconds=config.OTP_RESEND_COOLDOWN_SEC),
#             "attempts": 0,
#             "consumed": False,
#             "created_at": now(),
#         }
...
#     resp = {
#         "message": "OTP sent",
#         "resend_in": config.OTP_RESEND_COOLDOWN_SEC,
#     }
...
#     if (
#         not ch
#         or ch["expires_at"] <= now()
#         or ch["attempts"] >= config.OTP_MAX_ATTEMPTS
#     ):
#         raise HTTPException(
#             401,
...
#     # Clear push token so logged-out users no longer receive notifications
...
#         print(
#             "[LOGOUT PUSH DEBUG] "
#             f"push_token cleared for user={session['user_id']}"
#         )
...
#         radius = (
#             a.get("radius_km")
#             or settings["max_service_radius_km"]
#         )

#         d = finance.haversine_km(
#             body.lat,
#             body.lng,
#             a["lat"],
#             a["lng"],
...
# # Newly added fallback route to prevent 400 Bad Request
...
#         ser(f)
#         for f in foods
#         if f["restaurant_id"] in valid_rids
#     ]
...
#         {"_id": oid(body.restaurant_id)}
#     )

#     if (
#         not restaurant
#         or restaurant.get("status") != "approved"
#     ):
#         raise HTTPException(
#             404,
...
#         raise HTTPException(
#             400,
#             "Select a valid delivery address",
#         )

#     match = await match_area(
#         LatLng(
#             lat=address["lat"],
#             lng=address["lng"],
#         )
...
#         raise HTTPException(
#             403,
#             "BiteGo is not available at this address",
#         )

#     route = await maps.get_route(
#         restaurant["lat"],
#         restaurant["lng"],
#         address["lat"],
#         address["lng"],
...
#     serviceable = (
#         distance
#         <= settings["max_service_radius_km"]
#     )
...
#         restaurant_price = int(f["price"])

#         disc_price = f.get("discount_price")

#         if (
#             disc_price is not None
#             and int(disc_price) < restaurant_price
#         ):
#             restaurant_price = int(disc_price)

#         admin_margin = int(
#             f.get("admin_margin", 0)
#         )
...
#         restaurant.get(
#             "commission_pct",
#             settings["restaurant_commission_pct"],
#         ),
#         restaurant.get(
#             "fixed_fee",
#             settings["restaurant_fixed_fee"],
#         ),
#     )
...
#         "restaurant_address": restaurant.get(
#             "address"
#         ),
...
#         "service_area_id": restaurant.get(
#             "service_area_id"
#         ),
...
#         "settings_snapshot": {
#             "platform_charge": settings[
#                 "platform_charge"
#             ],
#             "delivery_base_first_km": settings[
#                 "delivery_base_first_km"
#             ],
#             "delivery_additional_per_km": settings[
#                 "delivery_additional_per_km"
#             ],
#             "delivery_partner_earning_slabs": settings[
...
#     saved = await db.orders.find_one(
#         {"_id": res.inserted_id}
#     )
...
#                     timeout=15.0
#                 ) as client_http:
#                     sms_response = await client_http.get(
#                         url,
#                         params=params,
#                     )
...
#                     logger.error(
#                         "Delivery OTP SMS failed: "
#                         "status=%s body=%s",
#                         sms_response.status_code,
#                         sms_response.text[:500],
#                     )
#                 else:
#                     logger.info(
#                         "Delivery OTP SMS sent to "
#                         "phone ending %s",
#                         str(
#                             user.get("phone", "")
#                         )[-4:],
...
#         restaurant["owner_id"],
#         "New order received!",
#         f"Order for ₹{totals['customer_total']} "
#         "— accept within 10 min.",
#         type_="new_order",
#         data={
#             "order_id": str(
#                 res.inserted_id
...
#     if config.IS_DEV:
#         response["dev_delivery_otp"] = delivery_otp
...
#     rows = await db.orders.find(q).sort(
#         "created_at",
#         -1,
#     ).skip(
#         (page - 1) * limit
...
# # Restaurant reminder interval. The existing 10-minute auto-cancel rule
# # remains controlled by RESTAURANT_ACCEPT_TIMEOUT_MIN in common.py.
# RESTAURANT_REMINDER_INTERVAL_SEC = 30
...
# async def timeout_worker():
#     """
#     Background worker for restaurant orders.

#     Existing business rule is preserved:
#     - A PLACED order is automatically cancelled after
#       RESTAURANT_ACCEPT_TIMEOUT_MIN (currently 10 minutes).
#     - While the order remains PLACED, the restaurant receives a
#       reminder push approximately every 30 seconds.
#     - As soon as the order changes from PLACED to ACCEPTED, REJECTED,
#       or CANCELLED, reminders stop automatically.
#     """
...
#             # Only orders still waiting for restaurant acceptance are
#             # considered. Once the status changes, this worker ignores them.
#             pending_orders = db.orders.find({
#                 "status": "PLACED",
#             })

#             async for o in pending_orders:
#                 order_id = o["_id"]
#                 created_at = o.get(
#                     "created_at",
#                     current_time,
...
#                 # =========================================================
#                 # 1. EXISTING 10-MINUTE AUTO-CANCEL RULE
#                 # =========================================================
...
#                     # Only the worker that actually changed the order to
#                     # CANCELLED sends the timeout notification.
#                     if result.modified_count == 1:
#                         await notify(
#                             o["customer_id"],
#                             "Order cancelled",
...
#                     # It was cancelled, so no restaurant reminder.
#                     continue

#                 # =========================================================
#                 # 2. RESTAURANT REMINDER EVERY ~30 SECONDS
#                 # =========================================================
#                 # The initial notification is already sent by create_order().
#                 # Therefore, the first reminder is intentionally sent only
#                 # after 30 seconds instead of immediately duplicating it.
#                 last_reminder = o.get(
#                     "restaurant_last_reminder_at"
#                 )

#                 if last_reminder is None:
#                     should_remind = (
#                         current_time - created_at
#                     ).total_seconds() >= RESTAURANT_REMINDER_INTERVAL_SEC
#                 else:
#                     should_remind = (
...
#                 # Atomically claim this reminder slot. This prevents duplicate
#                 # reminders if more than one backend worker/process handles the
...
#                         },
#                         {
#                             "restaurant_last_reminder_at": None,
#                         },
#                     ]
#                 else:
#                     reminder_filter[
...
#                         "$lte": current_time
#                         - timedelta(
#                             seconds=RESTAURANT_REMINDER_INTERVAL_SEC
#                         )
#                     }
...
#                     {
#                         "$set": {
#                             "restaurant_last_reminder_at": current_time,
#                         },
#                         "$inc": {
#                             "restaurant_reminder_count": 1,
#                         },
#                     },
#                 )
...
#                     (
#                         f"Order for ₹"
#                         f"{o.get('customer_total', o.get('total', ''))} "
#                         "is still waiting for acceptance."
#                     ),
#                     type_="new_order_reminder",
#                     data={
#                         "order_id": str(order_id),
#                         "action": "open_order",
#                         "status": "PLACED",
...
#         except Exception as e:
#             logger.error(
#                 "timeout_worker error: %s",
#                 e,
#             )

#         # Check frequently so the 30-second reminder interval stays close to
#         # the requested timing without scanning the database continuously.
...
# ==================== RENFLAIR SMS HELPERS ====================


async def _renflair_get(
    endpoint: str,
    params: dict,
    label: str,
) -> bool:
    """Send a GET request to Renflair and return True only on HTTP success."""
    if config.SMS_PROVIDER != "renflair":
        return True

    if not config.RENFLAIR_API_KEY:
        logger.error(
            "Renflair API key is not configured for %s",
            label,
        )
        return False
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client_http:
            response = await client_http.get(
                f"https://sms.renflair.in/{endpoint}",
                params={
                    "API": config.RENFLAIR_API_KEY,
                    **params,
                },
        ...
        logger.info(
            "Renflair %s response: status=%s body=%s",
            label,
            response.status_code,
            response.text[:1000],
        )
        ...
        if response.is_error:
            logger.error(
                "Renflair %s failed: status=%s body=%s",
                label,
                response.status_code,
                response.text[:500],
            )
...
async def send_login_otp_sms(
    ...
    """V1 OTP SMS for customer, delivery, and restaurant login/signup."""
...
async def send_delivery_otp_sms(
    ...
    """V1 OTP SMS sent to the customer for order delivery verification."""
    return await _renflair_get(
        "V1.php",
        {
            "PHONE": phone,
...
async def send_restaurant_new_order_sms(
    ...
    """V4 normal new-order SMS for the restaurant; no OTP is included."""
    return await _renflair_get(
        "v4.php",
        {
            "PHONE": phone,
        ...
        f"restaurant new order [order={order_id}]",
    )
...
async def otp_request(body: OtpRequest):
    ...
    if config.SMS_PROVIDER == "renflair":
        ...
        if not sent:
            # Invalidate the newly-created challenge so the user can request
            # a fresh OTP after the SMS provider failure.
            await db.otp_challenges.update_one(
                {
                    "phone": phone,
                    "role": role,
        ...
        logger.info(
            "Renflair login OTP sent successfully "
            "to phone ending %s for role=%s",
            phone[-4:],
            role,
        )
...
async def create_order(
    ...
    if config.SMS_PROVIDER == "renflair":
        sent = await send_delivery_otp_sms(
            phone=user.get("phone"),
            otp=delivery_otp,
        ...
        if not sent:
            # Do not cancel the order if SMS delivery fails. The existing
            # order/push-notification flow remains intact and the failure is
            # logged by the Renflair helper.
            logger.error(
                "Customer delivery OTP SMS could not be sent "
                "for order=%s",
                str(res.inserted_id),
            )
    ...
    # ==================== RESTAURANT NEW ORDER SMS ====================
    # V4 sends the normal restaurant order SMS. It does NOT contain the
    # customer's delivery OTP.

    if config.SMS_PROVIDER == "renflair":
        try:
            restaurant_owner = await db.users.find_one(
                {
            ...
            if restaurant_owner:
                restaurant_phone = restaurant_owner.get("phone")

            if not restaurant_phone:
                restaurant_phone = restaurant.get("phone")

            if restaurant_phone:
                sent = await send_restaurant_new_order_sms(
                ...
                if sent:
                    logger.info(
                        "Restaurant new-order V4 SMS sent to phone ending %s",
                        str(restaurant_phone)[-4:],
                    )
                else:
                    logger.error(
                        "Restaurant new-order V4 SMS could not be sent "
                        "for order=%s",
                        str(res.inserted_id),
                    )
            else:
                logger.error(
                    "Restaurant phone not found. Cannot send V4 new-order SMS. "
                    "restaurant_id=%s",
                    restaurant.get("_id"),
                )
            ...
            logger.error(
                "Restaurant new-order V4 SMS error: %s",
                e,
            )
