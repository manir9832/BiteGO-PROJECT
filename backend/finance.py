...
async def get_delivery_partner_settlements():
    """ডেলিভারি পার্টনার অনুযায়ী Daily, Weekly এবং Remaining আর্নিংস হিসাব করা"""
    now = datetime.now(timezone.utc)
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    week_start = today_start - timedelta(days=7)
    ...
    for partner in partners:
        ...
        # এখানে "$regex" ব্যবহার করা হয়েছে যাতে ছোট বা বড় হাতের ("delivered" / "DELIVERED") যাই হোক না কেন কাজ করে
        ...
        for order in orders:
            settled_val = order.get("partner_settled") or order.get("is_settled")
            # যদি টাকা পে করা না হয়ে থাকে (settled না হয়)
            if settled_val not in [True, "True", "true", 1, "1"]:
                ...
                order_date = order.get("created_at") or order.get("delivered_at")
                if order_date:
                    if order_date >= today_start:
                        daily_earnings += earning
                    if order_date >= week_start:
...
async def settle_delivery_partner(partner_id: str):
    """ডেলিভারি পার্টনারের পেমেন্ট ক্লিয়ার করে ব্যালেন্স 0 করে দেওয়া"""
    try:
    ...
    await db.orders.update_many(
        {
            "$or": [
                {"delivery_partner_id": partner_id},
                {"delivery_partner_id": obj_id},
                {"delivery_boy_id": partner_id},
                {"delivery_boy_id": obj_id}
            ],
            "status": {"$regex": "^delivered$", "$options": "i"},
            "partner_settled": {"$ne": True}
        },
        {"$set": {"partner_settled": True, "partner_settled_at": datetime.now(timezone.utc)}}
    )
    return {"status": "success", "message": "Delivery partner payment settled and reset to 0"}